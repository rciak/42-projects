...
/*   Updated: 2026/03/02 22:13:43 by reciak           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	not__so_busy_wait(int64_t t_stop, t_squad_end *s_end)
{
	...
	while (!time_to_say_goodbye(s_end))
	{
		...
		else if (TH_SINGLE_WAIT < waiting_time
			&& waiting_time <= 2 * TH_SINGLE_WAIT - TH_BUSY_WAIT)
...
static void	busy__wait(int64_t t_stop, t_squad_end *s_end)
{
	while (now() < t_stop && !time_to_say_goodbye(s_end))
		;
}
